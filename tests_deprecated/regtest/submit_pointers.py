...
from sigma_runes.sigma import hex_to_pointers, pointers_to_hex
...
def create_op_return_transaction(wallet_name, data) -> str:
    try:
        ...
        print(f"Transacción no gastada {utxo}")
...
def fetch_rune_txs(wallet_name: str, data: str) -> List[str]:
    ...
    try:
        ...
        for tx in tx_list:
            # Check if transaction has OP_RETURN 
            if 'vout' in tx:
...
proofs = [
        (1, b'reputation_proof_script_template_hash', [("proof_id", 100)], b'pointer', b'text/utf8', b'1')
    ]

# Crear una transacción con un OP_RETURN
op_return_data = pointers_to_hex(proofs)  # Rune.
print(f"op return data -> {op_return_data}")
...
print(f"Content -> {hex_to_pointers(op_return_data)}")
